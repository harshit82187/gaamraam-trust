...
namespace App\Http\Controllers\Institute\Ticket;
...
class TicketController extends Controller
{
    public function view(Request $req){
        $member = Auth::guard('institute')->user();
        $tickets = Ticket::where('type', 3)->where('user_id', $member->id)->orderBy('id', 'desc')->paginate(10);
        return view('institute.ticket.list',compact('tickets'));
    }

    public function add(Request $req){
        if($req->isMethod('get')){
            return view('institute.ticket.add');
        }else{
            // dd($req->all());
            ...
            return redirect()->route('institute.tickets.view')->with('success', 'Ticket submitted successfully!');
        }
    }

    public function info($id){
        $member = Auth::guard('institute')->user();
        $ticket = Ticket::where('id',decrypt($id))->where('user_id',$member->id)->where('type',3)->first();
        if($ticket == null){
        return back()->with('error','Ticket not found!');
        }
        return view('institute.ticket.view', compact('ticket'));
    }
