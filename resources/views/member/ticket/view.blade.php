                        ...
                        <div class="card">
                            <div class="student_ticket_header d-flex">
                                <div class="d-flex align-items-center gap-2">
                                    <a href="https://www.gaamraam.ngo/student/tickets"><i class="fa-solid fa-arrow-left"></i></a>
                                    <h5>Ticket Number</h5>
                                </div>
                            ...
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <table class="table table-bordered">
                                    <tbody>
                                        ...
                                        <tr>
                                            <th>Description</th>
                                            <td class="text-start">{!! nl2br(e($ticket->description)) !!}</td>
                                        </tr>

                                        <tr>
                                            <th>Created At</th>
                                            <td class="text-start">{{ $ticket->created_at->format('d M Y, h:i A') }}</td>
                                        </tr>

                                        @if($ticket->status == 2)
                                        <tr>
                                            <th>Ticket Closed Date</th>
                                            <td class="text-start">{{ $ticket->updated_at->format('d M Y, h:i A') }}</td>
                                        </tr>
                                        @endif
                                        @if($ticket->feedback != null && $ticket->rating != null)
                                        <tr>
                                            <th>Your Feedback</th>
                                            <td class="text-start">@for ($i = 1; $i <= 5; $i++)
                                                    @if ($i <= $ticket->rating)
                                                    ...
                                                    @foreach(json_decode($ticket->attachments, true) as $file)
                                                        ...
                                                        @if(in_array($extension, ['jpg', 'jpeg', 'png']))
                                                            <a href="{{ asset($file) }}" target="_blank">
                                                                <img src="{{ asset($file) }}" alt="attachment" style="width: 80px; height: auto; border: 1px solid #ccc; border-radius: 4px; margin: 4px;">
                                                            </a>
                                                        @elseif($extension == 'pdf')
                                                            <a href="{{ asset($file) }}" target="_blank" class="btn btn-sm btn-outline-dark m-1">
                                                                View PDF
                                                            </a>
                                                        @endif
                                                    @endforeach
                        ...
                        <div class="card mt-3">
                            <div class="card-body">
                                ...
                                @foreach($ticket->replies as $reply)       
                                    <div class="ticket_chat_card d-flex mb-3 {{ $reply->type == 3 ? 'justify-content-end' : 'justify-content-start' }}">
                                        <img src="{{ $reply->image ? asset($reply->image) : ($reply->type == 3 ? ($reply->memberDetail && $reply->memberDetail->image ? asset($reply->memberDetail->image) : asset('front/images/avatar-01.jpg')) 
                                        ...
                                        <div class="chat_card {{ $reply->type == 3 ? 'user' : 'admin' }}">
                                            <div class="arrow">
                                                <div class="outer"></div>
                                                <div class="inner"></div>
                                            </div>
                                ...
                                @if($ticket->status == 1)
                                <hr>
                                <form class="ticket_reply" id="ticket-reply" action="{{ route('member.tickets.ticket-reply', encrypt($ticket->id)) }}" method="POST" enctype="multipart/form-data">
                                    @csrf
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="custom_card">
           <form id="tickets-feedback" action="{{ route('member.tickets.feedback') }}" method="post">
            @csrf
