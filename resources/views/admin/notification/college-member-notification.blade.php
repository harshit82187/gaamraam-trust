...
<div class="card">
	<div class="card-header d-block d-md-flex align-items-center gap-2 justify-content-between">
		<div class="d-flex align-items-center gap-2">
			<h3 class="ml-0 fs-3 fs-sm-4">College Member Notifications</h3>
			<span class="count-circle" style="position:unset;">{{ count($notifications ) }}</span>
		</div>
		<div>
			<a class="btn btn-primary mt-0 mt-sm-2" href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#send-notification">Send Notification</a>
			<button class="btn btn-info mt-0 mt-sm-2" onclick="window.location.href='{{ url()->current() }}';">Reset</button>
		</div>
	</div>
	<br>
	<div class="card-body">
			...
			<table class="table align-middle table-nowrap table-hover table-striped table-responsive">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							@if(is_array($notification->institutes))
							{{ !empty($notification->institutes) ? implode(', ', $notification->institutes) : 'No College Member Assigned' }}
							@else
							{{ $notification->institutes }}
							@endif
						</td>
						<td>
					...
					@else
					<tr>
						<td colspan="6" class="text-center text-danger">Notification's Not Found</td>
					</tr>
					@endif
				</tbody>
			</table>
...
<div class="modal fade" id="send-notification" tabindex="-1" aria-labelledby="modalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content" >
			...
			<form action="{{ route('admin.send-college-member-notification') }}" method="POST" enctype="multipart/form-data">
				@csrf
				<div class="modal-body">
					<!-- Subject Field -->
					<div class="mb-3">
						<label for="subject" class="form-label">College Member Name<span class="text-danger">*</span></label>
						<select class="form-control select2" name="institute_id[]" multiple required>
