    ...
    <div style="width: 300px;height:380px; border: 2px solid #d6d6d6; border-radius: 15px; padding: 20px; background: white; margin-bottom:20px;">
      <div style="text-align: center;">
        @php
          ...
          $peofileImagePath = public_path($admin->image);
      ...
      <div style="text-align: center;">
        ...
        <h3 style="margin: 10px 0 5px; font-size: 16px; justify-content:center">{{ $admin->name }}</h3>
      </div>
      <table style="font-size: 14px; width: 100%; margin-top: 10px;">
        ...
        <tr>
          ...
          <td style="text-align: right;">{{ $admin->role->name ?? 'N/A' }}</td>
        </tr>
        <tr>
          <td><strong>ID:</strong></td>
          <td style="text-align: right;">{{ $admin->id ?? 'N/A' }}</td>
        </tr>
        <tr>
          <td><strong>Blood Group:</strong></td>
          <td style="text-align: right;">{{ $admin->blood_group ?? 'N/A' }}</td>
        </tr>
        <tr>
          <td><strong>Mobile:</strong></td>
          <td style="text-align: right;">{{ $admin->mobile_no ?? 'N/A' }}</td>
        </tr>
        <tr>
          <td><strong>Email:</strong></td>
          <td style="text-align: right;">{{ $admin->email ?? 'N/A' }}</td>
        </tr>
      </table>
    </div>
    ...
    <div style="width: 300px;height:380px; border: 2px solid #d6d6d6; border-radius: 15px; padding: 20px; background: white;">
      <div style="text-align: center;">
        <h3 style="margin-bottom: 10px; font-size: 16px;justify-content:center;">Scan QR Code</h3>
         @if($admin->qr_code_path)
          @php
            $imagePath = public_path($admin->qr_code_path);
