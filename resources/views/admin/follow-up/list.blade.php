...
<div class="card">
	...
	<div class="card-body">
		<div class="mt-3 mb-1">
			<table class="table align-middle table-nowrap table-hover table-responsive table-striped" id="meta-table-container">
					...
					<tr>
						<td>
							<div style="font-weight: bold;">
								...
								@if($meta->created_at == now())
									<button style="background-color: #ffb6c1; color: #fff; border: none; border-radius: 5px; padding: 2px 8px; font-size: 12px; margin-left: 6px; cursor: pointer;">New</button>
								@endif
							</div>
						...
						<td>
							<div style="font-weight: bold;">{{ $meta->telleCaller->name }}</div>
							<div>{{ $meta->telleCaller->mobile_no }}</div>
						</td>
						<td>
						...
						<td>
							...
							<div>
								<button class="follow-up-btn" style="background-color: #ffc107; color: #ffffff; border: none; padding: 5px 10px; border-radius: 4px; font-weight: bold;"
									data-id="{{  $meta->sarpanch->id }}" data-name="{{  $meta->sarpanch->name }}" data-type="admin">Follow Up ({{ $followUpCount ?? '0' }})</button>
							</div>
	...
	$(document).on('submit', '#followUpForm', function(e) {
	    ...
	    $.ajax({
	            ...
	            success: function(response) {
	                ...
	                $('#meta-table-container').load(location.href + ' #meta-table-container > *');
	                iziToast.info({
			title: 'Info',
